...
import (
	...
	"github.com/Nobsmoke123/snippetbox/internal/models/mocks"
...
func newTestApplication(t *testing.T) *application {
	...
	// Add a session manager instance. Note that we use the same settings as production
	// except that we don't set a store for the session manager.
	// If no store  is set, SCS package will default to using a transient in-memory store,
...
func (ts *testServer) postForm(t *testing.T, urlPath string, form url.Values)(int, http.Header, string){
	...
	// Read the response from the test server
	defer rs.Body.Close()
	body, err := io.ReadAll(rs.Body)
	if err != nil {
		t.Fatal(err)
